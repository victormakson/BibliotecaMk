...
def query(mydb):
    ...
    print("\n{: <20} {: <30} {: <10} {: <10}".format(
        "Título", "Autor", "Ano", "Status"))
    print("-" * 70)

    # Itera sobre cada linha retornada pela consulta e imprime os dados formatados
    for row in rows:
...
def register(mydb, nome, email, senha, data_nascimento, rua, bairro, cidade, estado, cep):
    mycursor = mydb.cursor()

    sql = "INSERT INTO usuarios (nome, email, senha, data_nascimento, rua, bairro, cidade, estado, cep) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
    val = (nome, email, senha, data_nascimento,
